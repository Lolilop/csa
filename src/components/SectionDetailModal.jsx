...
function SectionDetailModal({ section, isOpen, onClose }) {
  ...
  const handleRegisterClick = () => {
    navigate(`/inscription?sectionId=${section.nom}`);
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button 
          onClick={handleClose}
          className="absolute top-4 right-4 z-20 text-white bg-black bg-opacity-40 hover:bg-opacity-60 rounded-full p-2 transition-all duration-200 shadow-lg"
          aria-label="Fermer"
        >
        ...
        <div className="border-t border-gray-200 py-4 px-6 flex flex-col sm:flex-row justify-end items-center space-y-3 sm:space-y-0 sm:space-x-4 mt-auto">
          <button
            onClick={handleClose}
            className="w-full sm:w-auto px-5 py-2.5 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition-colors focus:outline-none focus:ring-2 focus:ring-gray-200 focus:ring-offset-2"
          >
            Fermer
          </button>
          <button
            onClick={handleRegisterClick}
            className={`w-full sm:w-auto px-5 py-2.5 ${colors.accent} rounded-lg text-white font-medium ${colors.hover} transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-${section.category === 'sport' ? 'emerald' : section.category === 'art' ? 'amber' : section.category === 'bien-être' ? 'violet' : 'blue'}-500`}
          >
