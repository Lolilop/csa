        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredEvents.length > 0 ? (
            filteredEvents.map((event) => (
              <div 
                key={event.id} 
                className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 hover-lift card-hover-effect cursor-pointer"
                onClick={() => handleShowEventDetails(event)}
              >
                <div className="h-48 relative overflow-hidden img-hover-zoom">
                  <img 
                    src={event.image} 
                    alt={event.title}
                    className="w-full h-full object-cover"
                  />
                  <div className="absolute top-0 right-0 bg-blue-600 text-white px-3 py-1 m-2 rounded-md font-semibold shadow-md">
                    {event.date}
                  </div>
                </div>
                <div className="p-5">
                  ...
                  <h3 className="text-xl font-bold mb-2 text-gray-800 group-hover:text-blue-600 transition-colors">{event.title}</h3>
